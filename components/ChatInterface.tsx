...
import { Send, Lock, ShieldCheck, Cpu, X, Plus, FileText, Download, Box, FileCode, Users, Wifi, WifiOff, Copy, Check, Share2, UserPlus, Trash2, User, Link as LinkIcon, CheckCheck } from 'lucide-react';
            ...
            {msg.sender === 'system' ? (
             ...
             <>
                ...
                <div className={`max-w-[85%] md:max-w-[70%] flex flex-col gap-1 ${msg.sender === 'user' && msg.senderProfile?.username === userProfile.username ? 'items-end' : 'items-start'}`}>
                    ...
                    <div
                    className={`rounded-[1.5rem] px-5 py-4 text-sm md:text-base leading-relaxed shadow-md backdrop-blur-sm border relative ${
                    ...
                    >
                        ...
                        <div className={`flex items-center justify-end gap-1 text-[10px] mt-2 font-mono ${msg.sender === 'user' && msg.senderProfile?.username === userProfile.username ? 'text-emerald-200/70' : 'text-zinc-500'}`}>
                            {msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            {msg.sender === 'user' && msg.senderProfile?.username === userProfile.username && (
                                <CheckCheck size={14} className="ml-1 text-emerald-400" />
                            )}
                        </div>
                    </div>
                </div>
